...
export async function computePopularCourts(): Promise<PopularCourt[]> {
  ...
  const rankedCourts: PopularCourt[] = facilityStatsList.map((facility, i) => {
    ...
    if (bookingRates[i] >= 0.5) {
      reasons.push('예약 경쟁 치열');
    }
    if (facility.is_free) {
      reasons.push('무료');
    }
    if (facility.review_count >= 3 && bayesianScores[i] >= 4.0) {
      reasons.push('높은 평점');
    }
    if (facility.favorite_count >= 3) {
      reasons.push('즐겨찾기 많음');
    }
    if (reasons.length === 0) {
      if (bookingRates[i] > 0) {
        reasons.push('예약 활발');
