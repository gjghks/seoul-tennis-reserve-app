...
export default function ReviewSection({ courtId, courtName, district }: ReviewSectionProps) {
  ...
  return (
    <section className={`${themeClass('bg-white dark:bg-slate-800 border-2 border-black dark:border-slate-700 rounded-[5px] shadow-[4px_4px_0px_0px_#000] overflow-hidden', 'bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700 overflow-hidden')} `}>
      <div className={`p-5 ${themeClass('border-b-2 border-black dark:border-slate-700', 'border-b border-gray-100 dark:border-slate-700')} `}>
        <div className="flex items-center justify-between">
          <h2 className={`font-bold flex items-center gap-2 ${themeClass('text-black dark:text-slate-100 uppercase', 'text-gray-900 dark:text-slate-100')} `}>
            <svg className={`w-5 h-5 ${themeClass('text-black dark:text-slate-100', 'text-green-600 dark:text-green-400')}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
          ...
          <div className={`p-6 text-center ${themeClass('bg-gray-100 dark:bg-slate-800 border-2 border-black dark:border-slate-700 rounded-[5px]', 'bg-gray-50 dark:bg-slate-900 rounded-xl')}`}>
            ...
            <button
              ...
              className={themeClass(
                ...
                'text-sm font-medium text-green-600 dark:text-green-400 hover:text-green-700 px-4 py-2 border border-gray-200 dark:border-slate-700 rounded-lg hover:border-green-300 transition-colors'
              )}
            >
              다시 시도
            </button>
