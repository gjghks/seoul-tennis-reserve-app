      ...
      <div className="max-w-4xl mx-auto mb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className={themeClass(
            ...
            <div className={`w-12 h-12 mb-4 flex items-center justify-center text-2xl ${themeClass('bg-[#ff90e8] border-2 border-black rounded-[5px]', 'bg-pink-50 rounded-xl text-pink-500')}`}>
              🏆
            </div>
            <h3 className={`text-xl mb-2 ${themeClass('font-black text-black', 'font-bold text-gray-900')}`}>자동 대진표</h3>
            <p className={themeClass('text-black/70 font-medium', 'text-gray-500')}>참가자 등록만 하면 시드 배정부터 대진표 생성까지 자동으로 완성됩니다.</p>
          </div>
          <div className={themeClass(
            'bg-white border-2 border-black rounded-[5px] shadow-[4px_4px_0px_0px_#000] p-6 hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-[2px_2px_0px_0px_#000] transition-all',
            'bg-white rounded-2xl border border-gray-100 p-6 shadow-sm hover:shadow-md transition-shadow'
          )}>
            <div className={`w-12 h-12 mb-4 flex items-center justify-center text-2xl ${themeClass('bg-[#22c55e] border-2 border-black rounded-[5px]', 'bg-green-50 rounded-xl text-green-500')}`}>
              🎯
            </div>
            <h3 className={`text-xl mb-2 ${themeClass('font-black text-black', 'font-bold text-gray-900')}`}>실시간 진행</h3>
            <p className={themeClass('text-black/70 font-medium', 'text-gray-500')}>점수를 입력하면 대진표가 실시간으로 업데이트됩니다.</p>
          </div>
          <div className={themeClass(
            'bg-white border-2 border-black rounded-[5px] shadow-[4px_4px_0px_0px_#000] p-6 hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-[2px_2px_0px_0px_#000] transition-all',
            'bg-white rounded-2xl border border-gray-100 p-6 shadow-sm hover:shadow-md transition-shadow'
          )}>
            <div className={`w-12 h-12 mb-4 flex items-center justify-center text-2xl ${themeClass('bg-[#88aaee] border-2 border-black rounded-[5px]', 'bg-blue-50 rounded-xl text-blue-500')}`}>
              📤
            </div>
            <h3 className={`text-xl mb-2 ${themeClass('font-black text-black', 'font-bold text-gray-900')}`}>쉬운 공유</h3>
            <p className={themeClass('text-black/70 font-medium', 'text-gray-500')}>카카오톡으로 대진표를 공유하고, 모두가 실시간으로 진행 상황을 확인합니다.</p>
          </div>
        </div>
